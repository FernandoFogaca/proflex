// bloco: novo agendamento / cadastro
import React, { useEffect, useMemo, useState } from "react";
import { useSearchParams, useNavigate } from "react-router-dom";
import { jsPDF } from "jspdf";
import { useApp } from "../context/AppContext.jsx";
import FotoCapture from "./FotoCapture.jsx";
...
// bloco: listas doença/áreas
const DOENCAS_COMUNS = ["Artrite","Artrose","Tendinite","Bursite","Hérnia de disco","Lombalgia","Cervicalgia","Escoliose","Osteoporose","Fibromialgia","Enxaqueca","Sinusite","Refluxo","Hipotireoidismo","Hipertireoidismo","Ansiedade","Depressão","Insônia","Dermatite","Varizes"];
const AREAS = {
...
export default function NovoAgendamento() {
  ...
  const [doencasExtras, setDoencasExtras] = useState([]);
  const [outrosDiagnosticos, setOutrosDiagnosticos] = useState("");
  const [remedios, setRemedios] = useState("");

  const [anotacoesPaciente, setAnotacoesPaciente] = useState("");
  const [observacoes, setObservacoes] = useState("");
  const [textoConsultaAtual, setTextoConsultaAtual] = useState("");
  ...
  const [prof, setProf] = useState(() => loadLS("profissional", { nome:"", registro:"", email:"", telefone:"", endereco:"" }));

  // bloco: foto perfil
  const [fotoPerfil, setFotoPerfil] = useState(clienteExistente?.fotoPerfilUrl || "");

  // bloco: preencher edição
  useEffect(() => {
    if (clienteExistente) {
      ...
      if (clienteExistente.areas) setAreas(clienteExistente.areas);
      setFotoPerfil(clienteExistente.fotoPerfilUrl || "");
  ...
  // bloco: toggles
  const toggleComorb = (n) => setComorb((p) => p.includes(n) ? p.filter(x=>x!==n) : [...p, n]);
  const toggleDoenca = (n) => setDoencasExtras((p) => p.includes(n) ? p.filter(x=>x!==n) : [...p, n]);
  const toggleAreaPart = (g, part) => setAreas((prev) => {
    const item = prev[g] || { parts: [], note: "" };
    const parts = item.parts.includes(part) ? item.parts.filter(x=>x!==part) : [...item.parts, part];
    return { ...prev, [g]: { ...item, parts } };
  });
  const setAreaNote = (g, note) => setAreas((prev) => ({ ...prev, [g]: { ...prev[g], note } }));

  // bloco: seleções consultas
  const selecionarTudo = (marcar) => setSel(marcar ? (consultas.map(c=>c.id)) : []);
  const novaConsulta = () => {
    if (!clienteExistente) { alert("Salve o paciente primeiro."); return; }
    const agora = new Date();
    addConsulta(clienteExistente.id, { dataISO: agora.toISOString().slice(0,10), hora: agora.toTimeString().slice(0,5), texto: "" });
  };
  const setTextoConsulta = (cid, texto) => { if (!clienteExistente) return; updateConsulta(clienteExistente.id, cid, { texto }); };
  ...
  const excluirSelecionadas = () => { if (!clienteExistente) return; deleteConsultas(clienteExistente.id, sel); setSel([]); };

  const idade = calcIdade(nascimento);

  // bloco: salvar cadastro/agenda
  const handleSubmit = (e) => {
    e.preventDefault();
    const baseCliente = { nome: nome || "Sem nome", telefone, email, endereco, anotacoesPaciente, fotoPerfilUrl: fotoPerfil || "" };
    const extras = tipo === "completo" ? { nascimento, peso, altura, genero, comorbidades: comorb, doencasExtras, outrosDiagnosticos, remedios, areas } : {};
    let cliente = clienteExistente;
    if (cliente) updateClient(cliente.id, { ...baseCliente, ...extras });
    else cliente = addClient({ ...baseCliente, ...extras, consultas: [], receitas: [], atestados: [] });

    const resumoExtras = (() => {
      const linhas = [];
      if (doencasExtras.length) linhas.push(`Doenças adicionais: ${doencasExtras.join(", ")}`);
      const blocos = Object.entries(areas).filter(([_,v]) => v.parts.length || v.note)
    ...
    if (textoConsultaAtual.trim()) {
      ...
      addConsulta(cliente.id, { dataISO, hora: horaOK, texto: resumoExtras + textoConsultaAtual.trim() });
    }
  ...
  // bloco: pdf receita/atestado
  const pdfHeader = (doc, titulo) => { doc.setFontSize(18); doc.text(titulo, 40, 40); doc.setLineWidth(0.5); doc.line(40, 48, 555, 48); doc.setFontSize(11); };
  const pdfFooterProf = (doc, y) => { y += 16; doc.line(40, y, 555, y); y += 18; doc.text(`${prof.nome || ""}  |  Registro: ${prof.registro || "-"}`, 40, y); y += 16; doc.text(`${prof.email || ""}  |  Tel: ${prof.telefone || ""}  |  End.: ${prof.endereco || ""}`, 40, y); };
  const imprimirReceita = (cliente, texto) => {
    ...
    updateClient(cliente.id, { receitas: [ ...(cliente.receitas || []), { id: `rec_${Date.now()}`, createdAt: new Date().toISOString(), texto } ] });
  };
  const imprimirAtestado = (cliente, para, texto) => {
    ...
    updateClient(cliente.id, { atestados: [ ...(cliente.atestados || []), { id: `ats_${Date.now()}`, createdAt: new Date().toISOString(), para, texto } ] });
  };

  // bloco: foto perfil ações imediatas (persistir ao remover)
  const removerFoto = () => {
    setFotoPerfil("");
    if (clienteExistente) updateClient(clienteExistente.id, { fotoPerfilUrl: "" });
  };
  const capturarFoto = (dataUrl) => {
    setFotoPerfil(dataUrl);
    if (clienteExistente) updateClient(clienteExistente.id, { fotoPerfilUrl: dataUrl });
  };
  ...
  return (
    <div className="container py-4">
      ...
      <div className="mb-2">
        <strong>Data selecionada:</strong> {toBRDate(data)} &nbsp;|&nbsp;
      ...
      {/* bloco: form topo com foto à direita */}
      <form onSubmit={handleSubmit}>
        <div className="row g-3 align-items-start">
          ...
          <div className="col-md-3">
            <label className="form-label">Foto do paciente</label>
            <div className="border rounded p-2 bg-white" style={{ position:"sticky", top:12 }}>
              {!fotoPerfil ? (
                <FotoCapture onData={capturarFoto} width={220} height={220} />
              ) : (
                <div className="text-center">
                  <img src={fotoPerfil} alt="perfil" style={{ width:220, height:220, objectFit:"cover", borderRadius:8 }} />
                  <button type="button" className="btn btn-sm btn-outline-danger mt-2 w-100" onClick={removerFoto}>Remover foto</button>
                </div>
              )}
            </div>
          </div>
          ...
          <>
            ...
            <div className="mt-3">
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:"8px" }}>
                {COMORBIDADES.map((n) => (
                  <label key={n} className="form-check" style={{ display:"flex", alignItems:"center", gap:6, border:"1px solid #eee", borderRadius:8, padding:"6px 10px" }}>
                    <input type="checkbox" className="form-check-input" checked={comorb.includes(n)} onChange={()=>toggleComorb(n)} />
                    <span>{n}</span>
                  </label>
                ))}
              </div>
            ...
            <div className="mt-2">
              ...
              <div style={{ display:"flex", flexWrap:"wrap", gap:"8px" }}>
                {DOENCAS_COMUNS.map((n) => (
                  <label key={n} className="form-check" style={{ display:"flex", alignItems:"center", gap:6, border:"1px solid #eee", borderRadius:8, padding:"6px 10px" }}>
                    <input type="checkbox" className="form-check-input" checked={doencasExtras.includes(n)} onChange={()=>toggleDoenca(n)} />
                    <span>{n}</span>
                  </label>
                ))}
              </div>
              ...
              {Object.entries(AREAS).map(([grupo, parts]) => (
                <div key={grupo} className="mb-2 p-2 border rounded">
                  ...
                  <div style={{ display:"flex", flexWrap:"wrap", gap:"8px", marginTop:8 }}>
                    {parts.map((p) => (
                      <label key={p} className="form-check" style={{ display:"flex", alignItems:"center", gap:6, border:"1px solid #eee", borderRadius:8, padding:"4px 8px" }}>
                        <input type="checkbox" className="form-check-input" checked={areas[grupo].parts.includes(p)} onChange={()=>toggleAreaPart(grupo, p)} />
                        <span>{p}</span>
                      </label>
                    ))}
                  </div>
                  {(areas[grupo].parts.length > 0) && (
                    <div className="mt-2">
                      <input className="form-control" placeholder={`Observações específicas para ${grupo} (opcional)`} value={areas[grupo].note} onChange={(e)=>setAreaNote(grupo, e.target.value)} />
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="mt-2">
          ...
          <textarea className="form-control" rows={4} placeholder="Evolução, diagnóstico, plano, doses/frequências e duração…" value={textoConsultaAtual} onChange={(e)=>setTextoConsultaAtual(e.target.value)} />
        </div>
        ...
        <>
          ...
          <div className="d-flex justify-content-between align-items-center mb-2">
            ...
            <div className="d-flex gap-2">
              <div className="form-check me-3">
                <input type="checkbox" className="form-check-input" id="selAll" checked={sel.length === consultas.length && consultas.length>0} onChange={(e)=>selecionarTudo(e.target.checked)} />
            ...
            {consultas.map((c) => (
              <div key={c.id} className="list-group-item">
                <div className="d-flex justify-content-between align-items-center">
                  <div><strong>Consulta {c.numero}</strong> — {toBRDate(c.dataISO)} {c.hora || ""}</div>
                  <input type="checkbox" className="form-check-input" checked={sel.includes(c.id)} onChange={()=>setSel((p)=>p.includes(c.id)?p.filter(x=>x!==c.id):[...p,c.id])} />
                </div>
                <textarea className="form-control mt-2" rows={4} placeholder="Edite as anotações desta consulta…" value={c.texto || ""} onChange={(e)=>setTextoConsulta(c.id, e.target.value)} />
              </div>
            ))}
          ...
          <div className="row g-2 mb-3">
            ...
            <div className="col-12 text-end">
              <button className="btn btn-outline-secondary" onClick={()=>{ saveLS("profissional", prof); alert("Dados do profissional salvos."); }}>Salvar dados do profissional</button>
            </div>
          </div>

          <div className="mb-3 p-3 border rounded">
            <h6>Receita</h6>
            <textarea className="form-control" rows={6} placeholder="Medicação, doses, frequência, duração…" id="txtReceita" />
            <div className="text-end mt-2">
              <button className="btn btn-dark" onClick={()=>imprimirReceita(clienteExistente, document.getElementById("txtReceita").value)}>Imprimir Receita (PDF)</button>
            </div>
          </div>

          <div className="mb-4 p-3 border rounded">
            <h6>Atestado</h6>
            <input className="form-control mb-2" placeholder="Destinatário (Pessoa/Empresa)" id="atsPara" />
            <textarea className="form-control" rows={6} placeholder="Texto do atestado…" id="atsTexto" />
            <div className="text-end mt-2">
              <button className="btn btn-dark" onClick={()=>{
                const para = document.getElementById("atsPara").value;
                const txt  = document.getElementById("atsTexto").value;
                imprimirAtestado(clienteExistente, para, txt);
              }}>Imprimir Atestado (PDF)</button>
            </div>
          </div>

          <div className="d-flex justify-content-end">
            <button className="btn btn-danger" onClick={()=>{ deleteClient(clienteExistente.id); navigate("/clientes"); }}>Excluir paciente</button>
          </div>
        </>
      )}
    </div>
